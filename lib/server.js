...
function serve (config) {
  ...
  app.get('/xxx.png', function *() {
    ...
    //logger.debug('url: ' + this.request.url);
    //var queryraw = require('url').parse(this.request.url, false).query;


    //logger.debug('querys: ', queryraw.cookie);
    //logger.debug('coooooook', queryraw.match(/cookie=([^&]*)/)[1]);
    //var query = queryraw;
    //result.cookies = decodeURIComponent(queryraw.match(/cookie=([^&]*)/)[1]);
    //logger.debug('cookiessssss: ', result.cookies);
